...
export default function KidsProducts() {
  ...
  const handleAddToWishlist = (e: React.MouseEvent, product: Product) => {
    e.preventDefault(); // Prevent default link behavior
    const isInWishlist = wishlist.find((item: { _id: string; }) => item._id === product._id);

    if (isInWishlist) {
      removeFromWishlist(product._id);
      toast.success(`${product.name} removed from wishlist!`);
      setWishlist(wishlist.filter((item: { _id: string; }) => item._id !== product._id));
    } else {
                  ...
                  <div className="relative aspect-square bg-slate-50  rounded-[4px] sm:p-4">
                    ...
                    <div className="absolute top-2 sm:top-3 right-2 sm:right-3 z-10 flex flex-col gap-2">
                      <Button 
                        ...
                        onClick={(e: any) => handleAddToWishlist(e, product)}
                      >
